...
# Verbindungsmethode abfragen
read -p "Möchtest du HTTP oder HTTPS verwenden? (http/https): " CONNECTION_TYPE

# Domain oder IP abfragen
read -p "Bitte gib die Domain oder IP an: " DOMAIN

# Nginx-Konfigurationsdatei erstellen
if [ "$CONNECTION_TYPE" == "https" ]; then

  # Nginx-Konfiguration für HTTPS erstellen
...
EOL
  ...
  # Nginx-Konfiguration für HTTP erstellen
  cat <<EOL | sudo tee /etc/nginx/sites-available/$DOMAIN
server {
    listen 80;
    server_name $DOMAIN;
...
# Nginx-Konfigurationsdatei aktivieren
